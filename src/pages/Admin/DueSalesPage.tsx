...
export default function DueSalesPage({
  ...
  const handleAdd = async (e?: React.FormEvent) => {
    ...
    const apiResult = await tryApi("POST", apiBase, newSale);
    setSourceData((prev) => [apiResult.ok ? apiResult.body ?? newSale : newSale, ...prev]);
    setForm(blankForm);
    setCurrentPage(1);
  ...
  const handleSaveEdit = async (e?: React.FormEvent) => {
    ...
    const apiResult = await tryApi("PUT", `${apiBase}/${editTargetId}`, updated);
    setSourceData((prev) => prev.map((p) => (p._id === editTargetId ? (apiResult.ok ? apiResult.body ?? updated : updated) : p)));
  ...
  const handleDelete = async () => {
    ...
    // Removed unused apiResult to fix TS6133
