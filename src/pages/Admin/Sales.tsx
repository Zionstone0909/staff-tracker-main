...
"use client"
import React, { useState, useEffect, useMemo, PropsWithChildren, CSSProperties, ChangeEvent } from 'react';
...
import {
    ...
    Timestamp, orderBy, DocumentReference, serverTimestamp, increment, FieldValue
} from 'firebase/firestore';
import { User, signOut } from 'firebase/auth';
import { useNavigate } from 'react-router-dom';
import {
    ShoppingCart, Plus, Minus, Trash2, CreditCard,
    Loader2, Search, ChevronDown, DollarSign
} from 'lucide-react';

/* ========================================================================== */
/* CONFIG & UI STYLES (INLINE CSS TEMPLATE - Consistent with Admin Pages)     */
/* ========================================================================== */

// --- Inline CSS Style Definitions ---
...
// --- Placeholder UI Components (for UI consistency) ---
...
// CartItem extends InventoryItem, inheriting 'unit_price'
...
interface LedgerTransaction {
    ...
    saleId?: string;
    paymentId?: string;
    amount: number; // Total value of the sale (Grand Total)
...
/* ========================================================================== */
/* CORE COMPONENT: SalesPage                                                  */
/* ========================================================================== */

const AdminSalesPage: React.FC = () => {
    const navigate = useNavigate();
    const [user, setUser] = useState<User | null>(null);
    const [userRole, setUserRole] = useState<'admin' | 'staff' | 'unknown'>('unknown'); // Role will be set to 'admin' based on route
    ...
    // --- Firebase Auth & Role Check (Protected Route) ---
    useEffect(() => {
        const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
            if (currentUser) {
                setUser(currentUser);
                // 🔶 1. Page Requirements: Assume 'admin' for /src/pages/Admin/Sales.tsx
                // In a production app, verify the role in Firestore here.
                setUserRole('admin'); 
            } else {
    ...
    // --- Data Processing & Calculations (🔶 2. Auto-calculating) ---
    ...
    const calculations = useMemo(() => {
        // Correctly use item.unit_price from the CartItem type
        ...
        const excess = Math.max(0, amountPaid - grandTotal); // Excess if paid more
    ...
    // --- Checkout Logic (🔶 3. Firestore Write Operations using runTransaction) ---
    const handleCheckout = async () => {
        ...
        try {
            await runTransaction(db, async (transaction) => {
                ...
                // 1. Check Inventory Stock and Update (🔶 3. B)
                ...
                // 2. Create Sale Record (🔶 3. A)
                const saleRecord: SaleRecord = {
                    saleId: saleRef.id,
                    customerId: selectedCustomer!.id,
                    customerName: selectedCustomer!.name,
                    // Map CartItem (unit_price) to SaleItem (price)
                    items: cart.map(item => ({ 
                        itemId: item.id, 
                        itemName: item.name, 
                ...
                // 3. Create Customer Ledger Entry (🔶 3. C)
                const ledgerStatus: LedgerTransaction["status"] = 
                    balance > 0 ? "owing" : // If balance > 0, tag as "owing"
                    excess > 0 ? "overpaid" : "paid"; // If excess > 0, tag as "overpaid"
                ...
                // Writes document to /customerLedger/{customerId}/transactions
                transaction.set(ledgerRef, ledgerEntry);

                // Update Customer's totalDue field for easy ledger lookup/search (Optional)
                if (balance > 0) {
    ...
    // 🔶 6. URL FIX: The user lands on /admin/sales, showing the Admin role context
    return (
        <div style={{ minHeight: '100vh', backgroundColor: LightBg }}>
            ...
            <main style={{
                ...
                <h1 style={{ fontSize: '1.875rem', fontWeight: 'bold', color: PrimaryColor }}>Point of Sale (ADMIN) <ShoppingCart size={28} style={{ verticalAlign: 'middle' }} /></h1>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr', gap: '2rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        ...
                        {/* 1. Customer Search & Selection (🔶 2. Sales Form Features) */}
                        <Card>
                        ...
                        {/* 2. Inventory Search & List (🔶 2. Adding sale items) */}
                        ...
                        <Card style={{ flex: 1 }}>
                            ...
                            <CardContent style={{ marginTop: '1rem' }}>
                                <div style={{ overflowY: 'auto', maxHeight: '200px' }}>
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                                ...
                                                <TableHead style={{ width: '30%', textAlign: 'right' }}>Total</TableHead>
                                            </TableRow>
                                        </TableHeader>
                                        <TableBody>
                                            {cart.map((item, index) => (
                                    ...
                                    {cart.length === 0 && (
                                        <p style={{ textAlign: 'center', color: MutedColor, padding: '1rem' }}>Cart is empty. Add items from the stock list.</p>
                                    )}
                                </div>
                            </CardContent>
                        </Card>
                        
                        {/* 4. Payment & Checkout Summary (🔶 2. Payment details) */}
                        <Card>
